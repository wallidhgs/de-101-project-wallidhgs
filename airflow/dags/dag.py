...
# @task
# def load_currency(df:pandas.DataFrame):
...
# 
#     df['currency_id'] = df.apply(lambda row: get_key(row['acronym']), axis=1)
#     new_df = df.filter(items=['currency_id', 'currency']).drop_duplicates()
#     insert_snowflake(new_df, table_name, columns, column_compare)
#     return new_df
# 
with DAG('de_project_dag', default_args=default_args, schedule_interval=None) as dag:
    product_list_s3 = list_products()
    dfs = load_products_file.expand(s3_file=product_list_s3)
    print(dfs)
    print(dfs)
    # for df, i in dfs:
        # load_currency(df=df, s3_file=product_list_s3[i])
